...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g_resp = r.get("gpt4o_response", "")
        g_str = (g_resp[:37] + "...") if len(g_resp) > 40 else g_resp
        m_resp = r.get("mini_response", "")
        m_str = (m_resp[:37] + "...") if len(m_resp) > 40 else m_resp
        g_lat = f"{r.get('gpt4o_latency', 0.0):.3f}s"
        m_lat = f"{r.get('mini_latency', 0.0):.3f}s"
        lines.append(f"{p_str:<40} | {g_str:<40} | {m_str:<40} | {g_lat:<15} | {m_lat:<15}")
    return "\n".join(lines)
